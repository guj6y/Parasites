...
%
 function [nicheweb,web_ip,Index_par,n,c,r,C_web]= ...
     NicheModelWithParasites_nk(num_species, connectance, num_parasites,...
...
nicheweb(max(basalsp),:);
n_basalmax = max(n(basalsp));
fraction_fp = 0.3;
fraction_pp = .09;
np_min = 0.2;%n_basalmax;
np_max = np_min+fraction_fp;
c_pmin = np_max*(1-fraction_pp)*fraction_fp;
...
%parameters for beta distribution:
    ...
    beta = (1-param)/param;
    r_p = betarnd(alpha,beta,num_parasites,1); %vector of ranges
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    while (tries_bis >0) && (ok_par_host==0)
        tries_bis=tries_bis-1;
         ...
         r_new_par = betarnd(alpha,beta,numel(zerorows_par),1)...
             .*(1-n_new_par);
