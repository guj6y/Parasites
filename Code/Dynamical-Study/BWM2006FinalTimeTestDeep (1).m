...
for Tf = Tfs
    ...
    for ii = 92:(nWeb);
        ...
        clc
        curTime = toc;
        fprintf('You are currently integrating web %u with 8 different values of Z\n',ii)
        ...
        for Z = ZAll
            ...
            sol = integrateBWM2006DeepTime(res,con,param);
            
            yFinals(:,count) = sol.yT;
            yMeans(:,count) = sol.mean;
            yStds(:,count) = sol.std;
            csvwrite(sprintf('%s/snapshots%04uZ%u.csv',resultsDir,ii-1,count),sol.snapshots);
            csvwrite(sprintf('%s/extinctions%04uZ%u.csv',resultsDir,ii-1,count),sol.extinctions);
            newerTime = toc;
