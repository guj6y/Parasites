function [sol] = integrateParasiteNullModelDeepTime(res,con,p)
...
snapshots = [5000,10000,25000,50000,100000,250000,500000];
sol.snaphsots = zeros(S,length(snapshots)+1);
snapshotsSaved = 0;
while tnow<Tfinal
    ...
    if tnow>snapshots(snapshotsSaved+1)
        snapshotsSaved = snapshotsSaved+1;
        sol.snapshots(:,snapshotsSaved) = sol.(partName).y(:,end);
    end
