...
for Tf = Tfs
    ...
    for ii = 1:(nWeb);
        ...
        clc
        curTime = toc;
        fprintf('You have tested %u of 4 Final Times.\n',whichTf-1)
        fprintf('You are currently integrating web %u with different values of Z; Tf = %u.\n',ii,Tf)
        ...
        for Z = ZAll
            ...
            yStds(:,count) = sol.std;
            save(sprintf('%s/sol%04uTf%u.mat',resultsDir,ii-1,Tf),'-struct','sol');
        end
        if saveResults
